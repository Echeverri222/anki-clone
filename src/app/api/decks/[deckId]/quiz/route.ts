...
export type QuizMode = 'image-to-text' | 'text-to-image';
...
export async function GET(
  ...
  try {
    ...
    for (let i = 0; i < Math.min(count, allCards.length); i++) {
      ...
      // Determine quiz mode (random if not specified)
      const questionMode: QuizMode = mode || (Math.random() > 0.5 ? 'image-to-text' : 'text-to-image');
